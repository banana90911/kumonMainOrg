trigger LeadTrigger on Lead (after insert) {
    ...
    for (Lead lead : Trigger.new) {
        ...
        for (User teacher : allTeachers) {
            ...
            // 리드 과목과 교사 과목이 일치하면 1, 아니면 0
            Decimal subjectScore = (teacher.SubjectInCharge__c == lead.Subject__c) ? 1 : 0;
        ...
        // 점수 정렬 내림차순
        teacherScores.sort((a, b) => b.score.compareTo(a.score));
        ...
        for (TeacherScoreWrapper tsw : teacherScores) {
            Task t = new Task(
                ...
                Status = '대기중',
                Priority = 'Normal',
                // hoursOffset / 24 : 한시간 간격
                ActivityDate = Date.today().addDays(hoursOffset / 24 / 20) 
            );
            tasksToInsert.add(t);
            hoursOffset++;
        }
...
public class TeacherScoreWrapper {
    public Id teacherId;
